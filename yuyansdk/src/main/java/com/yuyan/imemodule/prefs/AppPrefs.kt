...
class AppPrefs(private val sharedPreferences: SharedPreferences) {
    ...
    inner class Internal : ManagedPreferenceInternal(sharedPreferences) {
        ...
        val hwDictVersion = int("hw_dict_data_version", 0)  //缓存手写模型库版本号,用于校验是否覆盖模型文件
        val predictDbVersion = int("predict_db_data_version", 0)  //缓存联想预测词库版本号,用于校验是否覆盖 predict.db
